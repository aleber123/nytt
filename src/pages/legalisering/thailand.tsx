...
export default function ThailandLegaliseringPage() {
  ...
  const countryKey = 'thailand';
  const cp = `countryPages.${countryKey}`;
  
  const country = { name: 'Thailand', countryCode: 'TH', color: '#241D4F', colorDark: '#1a1539' };
  const embassy = {
      ...
      <main className="min-h-screen bg-gray-50">
        <section className="text-white py-16 md:py-24" style={{ background: `linear-gradient(to bottom right, ${country.color}, ${country.colorDark})` }}>
          <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="/bestall" className="bg-[#F4D03F] hover:bg-[#e5c236] text-black font-semibold px-8 py-4 rounded-lg">{t('countryPages.orderLegalization')}</Link>
        ...
        <section className="py-16 text-white" style={{ backgroundColor: country.color }}>
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <Link href="/bestall" className="bg-[#F4D03F] text-black font-semibold px-8 py-4 rounded-lg inline-block">{t('countryPages.orderNow')}</Link>
          </div>
        </section>
      </main>
    </>
