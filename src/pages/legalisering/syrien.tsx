...
export default function SyrienLegaliseringPage() {
  ...
  const embassy = {
    name: 'Syriens ambassad i Stockholm',
    address: 'Vendevägen 90, 5:e våningen',
    postalCode: '182 32 Danderyd',
    phone: '+46 8 622 18 70',
    email: 'info@syrianembassy.se',
    openingHours: 'Måndag-Fredag 09:00-15:00'
  };

  const documents = ['Personbevis och födelsebevis', 'Vigselbevis och familjedokument', 'Examensbevis', 'Fullmakter', 'Bolagshandlingar', 'Juridiska dokument'];

  return (
    <>
      <Head>
        <title>Legalisering för {country.name} - Ambassad Stockholm | DOX Visumpartner</title>
        <meta name="description" content={`Vi hjälper dig med legalisering av dokument för ${country.name}. Komplett service via ${country.name}s ambassad i Danderyd.`} />
