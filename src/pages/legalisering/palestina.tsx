...
export default function PalestinaLegaliseringPage() {
  const country = { name: 'Palestina', flag: '🇵🇸', color: '#007A3D', colorDark: '#005a2d' };
  const embassy = {
    name: 'Palestinas ambassad i Stockholm',
    address: 'Rådmansgatan 48',
    postalCode: '113 57 Stockholm',
    phone: '+46 997 200 749',
    email: 'embassyse@mofae.gov.ps',
    openingHours: 'Måndag-Fredag 09:00-15:00'
  };

  const documents = ['Personbevis och födelsebevis', 'Vigselbevis', 'Examensbevis', 'Fullmakter', 'Familjedokument', 'Juridiska dokument'];

  return (
    <>
      <Head>
        <title>Legalisering för {country.name} - Ambassad Stockholm | DOX Visumpartner</title>
        <meta name="description" content={`Vi hjälper dig med legalisering av dokument för ${country.name}. Komplett service via Palestinas ambassad i Stockholm.`} />
        <link rel="canonical" href="https://www.doxvl.se/legalisering/palestina" />
      </Head>
      <Header />
