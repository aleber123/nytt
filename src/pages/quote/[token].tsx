            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                  ...
                  <tbody>
                    {quote.lineItems.map((item, idx) => (
                      <tr key={idx} className="border-b border-gray-100 last:border-b-0">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-600 text-right">{item.unitPrice.toLocaleString()} kr</td>
                        <td className="px-4 py-3 text-sm font-semibold text-gray-900 text-right">{item.total.toLocaleString()} kr</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="bg-teal-600 px-4 py-4 flex justify-between items-center">
                <span className="text-white font-bold text-lg">
                  {isSwedish ? 'Totalbelopp' : 'Total Amount'}
                </span>
                <span className="text-white font-bold text-2xl">
                  {quote.totalAmount.toLocaleString()} kr
                </span>
              </div>

              <p className="text-xs text-gray-400 text-center py-2">
                {isSwedish ? 'Alla priser exkl. moms om inte annat anges' : 'All prices excl. VAT unless otherwise stated'}
              </p>
            </div>
          )}
